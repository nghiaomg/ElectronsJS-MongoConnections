...
document.addEventListener("DOMContentLoaded", () => {
  ...
  connectButton.addEventListener("click", async () => {
    ...
    try {
      ...
      const databases = await window.electronAPI.connectToMongoDB(
        connectionString
      );
...
function showRenameDatabaseModal(dbName) {
  ...
  document
    .getElementById("rename-db-btn")
    .addEventListener("click", () => renameDatabase(dbName));
  document
    .getElementById("cancel-rename-db")
...
async function renameDatabase(oldDbName) {
  ...
  try {
    ...
    const success = await window.electronAPI.renameDatabase({
      connectionString,
      oldDbName,
      newDbName,
    });
...
function showDeleteDatabaseConfirmation(dbName) {
  ...
  document
    .getElementById("confirm-delete-db")
    .addEventListener("click", () => deleteDatabase(dbName));
  document
    .getElementById("cancel-delete-db")
    .addEventListener("click", closeModal);
...
function showCreateDatabaseModal() {
  ...
  document
    .getElementById("create-db-btn")
    .addEventListener("click", createNewDatabase);
  document
    .getElementById("cancel-create-db")
    .addEventListener("click", closeModal);
...
async function createNewDatabase() {
  ...
  if (!dbName || !collectionName) {
    errorElement.textContent =
      "Database name and collection name cannot be empty";
    errorElement.style.display = "block";
    return;
  }
...
function showCreateCollectionModal(dbName) {
  ...
  document
    ...
    .addEventListener("click", () => createNewCollection(dbName));
  document
...
async function createNewCollection(dbName) {
  ...
  try {
    const connectionString = document.getElementById("connection-string").value;
    const success = await window.electronAPI.createCollection({
      connectionString,
      dbName,
      collectionName,
    });
...
function showCollectionContextMenu(event, dbName, collectionName) {
  ...
  contextMenu
    .querySelector(".rename-collection")
...
function showRenameCollectionModal(dbName, collectionName) {
  ...
  document
    .getElementById("rename-collection-btn")
    .addEventListener("click", () => renameCollection(dbName, collectionName));
  document
    .getElementById("cancel-rename-collection")
    .addEventListener("click", closeModal);
...
async function renameCollection(dbName, oldCollectionName) {
  ...
  try {
    ...
    renameButton.textContent = "Renaming...";
    const connectionString = document.getElementById("connection-string").value;
    const success = await window.electronAPI.renameCollection({
      connectionString,
      dbName,
      oldCollectionName,
      newCollectionName,
...
function showDeleteCollectionConfirmation(dbName, collectionName) {
  ...
  document
    .getElementById("confirm-delete-collection")
    .addEventListener("click", () => deleteCollection(dbName, collectionName));
  document
    .getElementById("cancel-delete-collection")
    .addEventListener("click", closeModal);
...
async function deleteCollection(dbName, collectionName) {
  try {
    const connectionString = document.getElementById("connection-string").value;
    const success = await window.electronAPI.deleteCollection({
      connectionString,
      dbName,
      collectionName,
    });
...
async function loadDocuments(dbName, collectionName) {
  ...
  try {
    ...
    const connectionString = document.getElementById("connection-string").value;
    const documentsString = await window.electronAPI.getDocuments({
      connectionString,
      dbName,
      collectionName,
    });
...
function displayDocuments(documents) {
  ...
  if (Array.isArray(documents) && documents.length > 0) {
    ...
    documentView.innerHTML =
      "<p>No documents found or invalid data received.</p>";
...
function showDeleteConfirmationModal(documentId) {
  ...
  document
    ...
    .addEventListener("click", () => deleteDocument(documentId));
  document
...
function displayJSONView(documents) {
  const documentView = document.getElementById("document-view");
  documentView.innerHTML = documents
    .map(
      (doc) => `
